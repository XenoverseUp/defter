...
import {
  ...
  StudentResourceData,
} from "@/lib/client-services/resources"
import {
...
function Resource({
  ...
  const progress =
    (data.totalQuestions - data.questionsRemaining) / data.totalQuestions
  ...
  return (
    <div className="px-4 pt-2 pb-2.5 space-y-1 relative">
      <div className="flex gap-2 items-center">
        <h4 className="font-semibold capitalize tracking-tight">
          {data.title}
        </h4>
      ...
      <div className="grow w-full flex items-center gap-2">
        <p className="text-sm">
          <span className="font-medium">{data.questionsRemaining}</span>{" "}
          questions remained out of{" "}
      ...
      <Button
        ...
        size="icon"
        variant="secondary"
        className="ml-auto absolute right-5 top-1/2 -translate-y-1/2"
      >
        <If
