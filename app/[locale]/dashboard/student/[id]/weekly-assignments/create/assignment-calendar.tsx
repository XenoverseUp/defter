...
function Entry({ item }: { item: Assignment }) {
  ...
  return (
    <div className={cn("w-full overflow-hidden relative group border rounded-sm", color.bg, color.text, color.border)}>
      <div className="flex flex-col h-full divide-y divide-inherit transition-opacity group-hover:opacity-0 select-none pointer-events-none">
        <header className="px-2 pb-2 pt-1.5">
          <span className="text-xs font-semibold line-clamp-1 mb-1.5 flex items-center gap-1">
            {subjectIconMap[resource.subject]}
            {tSubject(resource.subject)}
          </span>
          <h3 className="text-xs font-medium whitespace-nowrap text-ellipsis line-clamp-1">{resource?.title}</h3>
          <p className="text-xs line-clamp-1">{resource.press}</p>
        </header>

        <p className={cn("grow flex items-center justify-center text-xs px-2 py-1", color.content)}>
      ...
      <div
        ...
        onClick={() => setAssignments((assignments) => assignments.filter(({ resourceId }) => resourceId !== item.resourceId))}
      >
        <XIcon />
...
const subjectColors: Record<string, { bg: string; text: string; content: string; border: string }> = {
  // Middle School
  "social-studies": {
    bg: "bg-yellow-100",
...
const subjectIconMap: Record<(typeof subjectEnum.enumValues)[number], ReactNode> = {
