    ...
    <div className="h-full flex flex-col">
      <div className="grid grid-cols-7 gap-px sticky top-0 bg-background border-b">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn("mb-2 text-right text-sm text-muted-foreground pr-2", [0, 6].includes(i) && "text-muted-foreground/50")}
          >
            {day}
          </div>
        ))}
...
const CalendarCurrentDate = () => {
  ...
  return (
    <time suppressHydrationWarning dateTime={date.toISOString()} className="tabular-nums">
